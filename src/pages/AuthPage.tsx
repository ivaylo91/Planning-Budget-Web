...
import { IconCart, IconUser, IconMail, IconLock, IconEye, IconEyeSlash, IconCheck } from '../lib/icons'
...
function fieldClasses(hasError: boolean, shaking: boolean): string {
  const border = hasError || shaking ? 'border-red-500' : 'border-app-border focus-within:border-accent'
  return `relative flex items-center rounded-2xl border-[1.5px] bg-app-bg transition-colors ${border} ${shaking ? 'shake' : ''}`
}
...
export default function AuthPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-app-bg px-5 py-10">
      <div
        className="w-full max-w-sm rounded-[28px] bg-app-card border border-app-border p-6 sm:p-7 transition-opacity duration-200"
      ...
      >
        ...
        <div className="flex flex-col items-center text-center pb-6">
          ...
          <div className="text-2xl font-extrabold tracking-tight text-app-text">Пазарувай умно</div>
        ...
        {mode === 'login' && (
          ...
          >
            ...
            <button
              ...
              className="self-end -mt-1 text-[13px] font-semibold text-accent disabled:opacity-60"
            >
            ...
            {notice && (
              <p className="text-sm text-accent bg-accent/10 border border-accent/25 rounded-xl px-3.5 py-2.5 m-0">{notice}</p>
            )}

            <button
              type="submit"
              disabled={busy}
              className="pressable flex items-center justify-center rounded-2xl text-white text-[15px] font-bold py-[15px] min-h-[50px] shadow-[0_4px_16px_rgba(232,113,42,0.3)] disabled:cursor-not-allowed"
              style={{ background: busy ? 'var(--color-accent)' : 'linear-gradient(135deg, var(--color-accent), var(--color-accent-dark))', opacity: busy ? 0.7 : 1 }}
            >
              {busy ? <span className="spinner" /> : 'Вход'}
            </button>
        ...
        {mode === 'register' && (
          ...
          >
            ...
            <button
              type="submit"
              disabled={busy}
              className="pressable flex items-center justify-center rounded-2xl text-white text-[15px] font-bold py-[15px] min-h-[50px] shadow-[0_4px_16px_rgba(232,113,42,0.3)] disabled:cursor-not-allowed mt-0.5"
              style={{ background: busy ? 'var(--color-accent)' : 'linear-gradient(135deg, var(--color-accent), var(--color-accent-dark))', opacity: busy ? 0.7 : 1 }}
            >
              {busy ? <span className="spinner" /> : 'Създай акаунт'}
            </button>
