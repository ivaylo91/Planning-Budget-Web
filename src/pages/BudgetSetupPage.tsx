...
export default function BudgetSetupPage() {
  ...
  return (
    <div className="space-y-1">
      <h1 className="text-[22px] font-display font-bold text-app-text tracking-tight py-2 m-0">Бюджет</h1>

      {/* Period toggle */}
      <div className="flex p-1 rounded-full bg-app-card shadow-diffused mb-4">
        {(['weekly', 'monthly'] as const).map((type) => (
          <button
            key={type}
            type="button"
            onClick={() => handlePeriodChange(type)}
            className={`pressable flex-1 py-2.5 rounded-full text-sm font-semibold transition-colors ${
      ...
      {budget.amount > 0 && (
        <Reveal>
          <Bezel variant="full" className="bg-app-card px-5 py-6 text-center mb-4">
            <div className="relative inline-block mb-4">
              ...
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-center">
                <div className={`text-[28px] font-display font-extrabold tracking-tight tabular-nums ${pct > 90 ? 'text-red-500' : 'text-app-text'}`}>{pct}%</div>
            ...
            <div className="flex justify-center items-center gap-5">
              <div className="text-center">
                <div className="text-xl font-display font-extrabold tracking-tight text-accent tabular-nums">{formatEUR(spent)}</div>
                <div className="text-xs text-app-text-sec mt-0.5">Похарчени</div>
              </div>
              <div className="w-px h-9 bg-app-border" />
              <div className="text-center">
                <div className={`text-xl font-display font-extrabold tracking-tight tabular-nums ${remaining >= 0 ? 'text-green-500' : 'text-red-500'}`}>
                  {formatEUR(remaining)}
                </div>
      ...
      {/* Set budget */}
      <Bezel variant="full" className="bg-app-card mb-4">
        <form onSubmit={handleSubmit} className="px-5 py-[18px]">
          <div className="text-[15px] font-display font-bold text-app-text mb-3">Задай бюджет</div>
          <div className="flex items-center gap-2 mb-3.5">
            ...
            <input
              ...
              className="flex-1 text-xl font-bold text-app-text px-3.5 py-2.5 rounded-xl bg-app-bg focus:outline-none focus:ring-2 focus:ring-accent/40"
            />
          </div>
          ...
          <div className="flex gap-2 mb-1">
            {PRESETS[budget.period].map((preset) => (
              <button
                ...
                className={`pressable flex-1 py-2.5 rounded-full text-[13px] font-semibold transition-colors ${
                  budget.amount === preset
                    ? 'bg-accent text-white'
                    : 'bg-app-bg text-app-text-sec'
                }`}
              >
                €{preset}
              </button>
          ...
          <div className="mt-3.5">
            ...
            <input
              ...
              className="w-full rounded-xl bg-app-bg px-3 py-2 text-sm text-app-text focus:outline-none focus:ring-2 focus:ring-accent/40"
            />
          </div>

          <button
            type="submit"
            className="pressable w-full mt-4 rounded-full bg-accent text-white text-sm font-semibold py-2.5 hover:bg-accent-dark transition-colors"
          >
            Запази бюджета
          </button>
          {saved && (
            <p className="flex items-center justify-center gap-1.5 text-sm text-accent text-center mt-2">
              <IconCheck size={14} color="var(--color-accent)" /> Бюджетът е запазен
            </p>
          )}
        </form>
      </Bezel>
      ...
      {/* Tip */}
      <div className="rounded-2xl px-[18px] py-4 mb-4 bg-accent/10">
        <div className="text-sm font-display font-bold text-app-text mb-1">Съвет</div>
      ...
      {notificationsSupported() && (
        <Bezel variant="flat" className="bg-app-card p-5 space-y-2">
          <div className="flex items-center justify-between gap-3">
            <div>
              <h3 className="text-sm font-display font-bold text-app-text">Известия в браузъра</h3>
            ...
            <button
              ...
              className={`pressable shrink-0 flex items-center gap-1.5 rounded-full px-4 py-2 text-sm font-semibold transition-colors ${
                notifEnabled ? 'bg-accent text-white' : 'bg-app-bg text-app-text-sec'
              }`}
            >
              {notifEnabled && <IconCheck size={14} color="#fff" />}
